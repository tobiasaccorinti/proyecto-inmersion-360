...
import type { Experiencia, ReputacionEmpresa } from '../types'
...
interface ExperienciaCardProps {
  ...
  reputacion?: ReputacionEmpresa | null
}

function ReputacionBadge({ rep }: { rep: ReputacionEmpresa }) {
  if (rep.total_feedbacks === 0) {
    return (
      <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-500">
        ✨ {rep.total_experiencias === 1 ? 'Primera experiencia' : 'Sin reseñas aún'}
      </span>
    )
  }
  const estrellas = Math.round(rep.promedio ?? 0)
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-50 text-amber-600">
      {'★'.repeat(estrellas)}{'☆'.repeat(5 - estrellas)} {rep.promedio} ({rep.total_feedbacks})
    </span>
  )
}
    ...
    >
      ...
      <div className="p-5">
        <div className="flex items-center justify-between mb-2">
          ...
          {mostrarEstado && (
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${ESTADO_COLOR[experiencia.estado] ?? ''}`}>
