...
export const authService = {
  async register(dto: RegisterDto) {
    ...
    if (dto.role === 'estudiante') {
      ...
      await institucionesService.crear(userId, dto.nombreInstitucion, dto.tipoInstitucion ?? 'general')

    } else {
      // empresa — queda pendiente de validación por admin
      const { error: profileError } = await supabase
        .from('profiles')
        .insert({
          id: userId,
          ...
          documento_url: dto.documentoUrl ?? null,
        })
